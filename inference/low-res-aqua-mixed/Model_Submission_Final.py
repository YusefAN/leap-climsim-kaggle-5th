...
# model.load_state_dict(torch.load(DATA_PATH+'model_0.pt'))
...
index = np.where(sy <= MIN_STD_Y*1.01)
...
#         print(y_true[:, i])
#         print(all_outputs[:, i])
#         print(r2score(all_outputs[:, i], y_true[:, i]*10))
#         predt[:,i] = 0
...
# FFNN_LSTM_749_AVG_ATT_1e-15_ema_30
...
# ss_1 = pd.read_parquet(DATA_PATH + "ensemble_1e-15.parquet")
# ss_2 = pd.read_parquet(DATA_PATH + "sub_2.parquet")
# ss[list(TARGET_COLS)] = ss[list(TARGET_COLS)]*0.7 + ss_2[list(TARGET_COLS)]*0.3
...
# test_polars = pl.from_pandas(ss[["sample_id"]+list(TARGET_COLS)])
# test_polars.write_csv(DATA_PATH + "submission.csv")

# ss[["sample_id"]+list(TARGET_COLS)].to_parquet(DATA_PATH + "ensemble_1e-15.parquet")
# test_polars.write_csv("submission.csv")

# ss_1 = pd.read_parquet(DATA_PATH + "ensemble_1e-15.parquet")
# ss_2 = pd.read_parquet(DATA_PATH + "sub_2.parquet")
# ss_1[list(TARGET_COLS)] = ss_1[list(TARGET_COLS)]*0.65 + ss_2[list(TARGET_COLS)]*0.35
