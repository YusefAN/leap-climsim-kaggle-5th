# This script is a continuation of training of the base model using only the lowres dataset
...
from utils.utils import reduce_memory_usage_pl
from pytorch_lightning.callbacks import EarlyStopping, ModelCheckpoint
from pytorch_lightning.loggers import MLFlowLogger
...
#USE ONLY LOW-RES
ALL_FILES=[f'./lowres_torch/{i}' for i in os.listdir('./lowres_torch/') if 'pt' in i] #+ [f'./ocean_torch/{i}' for i in os.listdir('./ocean_torch/') if 'pt' in i]
NUM_ROWS = len(ALL_FILES)
print(NUM_ROWS)
NUM_VAL=4000
DATA_MAP = {k:v for k,v in zip(range(NUM_ROWS),ALL_FILES)}
values_loaded = torch.load("./preprrocessing/mean_std_values_main.pth")
X_MEAN = values_loaded['X_MEAN']
X_STD = values_loaded['X_STD']
Y_MEAN = values_loaded['Y_MEAN']
Y_STD = values_loaded['Y_STD']

def setup_mlflow_tracking(experiment_name, run_name,params, uri="http://95.179.166.204:5000"):
    mlflow.end_run()
...
class LeapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.cat(
            [
                torch.cat(
                    [x[:, :360].reshape(1000, 6, 60), x[:, 376:].reshape(1000, 3, 60)], dim=1
                ).permute(0, 2, 1),
                x[:, 360:376].unsqueeze(1).repeat(1, 60, 1).view(1000, 60, 16),
            ],
            -1,
        )
...
N_SEQ_OUTPUTS = len(TARGET_NAMES) // 60
ds_data = LeapDataset(
...
ds_train, ds_valid = random_split(ds_data, [len(ds_data)-NUM_VAL, NUM_VAL])
...
torch.save(values_to_save, './mean_std_values.pth')
...
class LeapModel(LightningModule):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=2):
        ...
        self.fc_lstm = MLP([32 * 60, 16 * 60, output_dim])
...
setup_mlflow_tracking(experiment_name="Submissions", run_name="0.7821_continue", params=params)

early_stopping_callback = EarlyStopping(
    monitor="val_score", mode="max", patience=3, verbose=True
)
...
trainer = Trainer(
    ...
    precision=16,
    callbacks=[checkpoint_callback, early_stopping_callback,StochasticWeightAveraging(swa_lrs=1e-4, swa_epoch_start=15)],
)

torch.manual_seed(41)
torch.set_float32_matmul_precision('medium')
trainer.fit(model, train_loader, valid_loader, ckpt_path='./val_score0.7489-best_model.ckpt')
